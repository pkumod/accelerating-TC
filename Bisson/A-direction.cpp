...
struct reorderUnit{
	...
	int degree;
	int id;
	bool operator < (const reorderUnit b) const {
		return (priority < b.priority)||(priority == b.priority && degree < b.degree)||
			   (priority == b.priority && degree == b.degree && id < b.id);
	}
};
int * peel(int *degreeRecord, std::vector<std::pair<int,int> > & edges, unsigned int edgeNum, int nodeNum) {
	...
	for (int t = 0; ; t ++) {
		...
		for (int i = 0; i < nodeNum; i ++) {
			if (runTimeDegree[i] <= threshold && priority[i] == -1) {
				...
				degreeRecord[i] = runTimeDegree[i];
			}
		}
		clock_t startTime = clock();
		priorityNum++;
		while (Frontier->size() > 0) {
			...
			//		output << curNode << " " << node << '\n';
						...
						if (runTimeDegree[node] <= threshold) {
							...
							degreeRecord[i] = runTimeDegree[node];
							peeledOffNodes++;
...
//#pragma omp parallel for
	...
	int *priorities = peel(degreeRecord,originEdges,totalEdge,nodeNum);
	//output graph
	/*
	for (int i = 0; i < nodeNum; i ++) {
	//	if (priorities[i] == -1)
	...
	struct reorderUnit * units = new reorderUnit[nodeNum];
	for (int i = 0; i < nodeNum; i ++) {
		units[i].id = i;
		units[i].priority = priorities[i];
		units[i].degree = degreeRecord[i];
	}
	std::sort(units, units + nodeNum);
	int * lookUpTable = new int [nodeNum];
	for (int i = 0; i < nodeNum; i ++) {
		lookUpTable[units[i].id] = nodeNum-1-i;
	}
	for (int i = 0; i < nodeNum; i ++) {
		for (int j = 0; j < neighbors[i].size(); j ++) {
			int curNbr = neighbors[i][j];
			outputLPGraphBIN.write((char*)&lookUpTable[i],sizeof(int));
			outputLPGraphBIN.write((char*)&lookUpTable[curNbr],sizeof(int));
		}
	}
	delete [] lookUpTable;
	delete [] units;
